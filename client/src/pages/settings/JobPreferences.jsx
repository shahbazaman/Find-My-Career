  ...
  const handleSubmit = async (e) => {
                ...
                <div className="multi-select-grid">
                  {preferences.roles.map(role => (
                    <button
                      ...
                      className="multi-tag selected indigo-selected"
                    >
                      <CheckCircle className="w-4 h-4" />
                      {role}
                      <X className="w-3 h-3" style={{marginLeft: '4px'}} />
                    </button>
                  ))}
                </div>
              )}
              ...
              <div ref={roleRef} style={{position: 'relative'}}>
                <div className="input-wrapper">
                  ...
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d={roleDropdownOpen ? "M18 15l-6-6-6 6" : "M6 9l6 6 6-6"} strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </button>
                </div>
                ...
                {roleDropdownOpen && (
                  <div className="dropdown-list">
                    {jobRoles
                      .filter(r => r !== "Select a role..." &&
                        r.toLowerCase().includes(roleSearch.toLowerCase()) &&
                        !preferences.roles.includes(r))
                      .map(role => (
                        <button
                          key={role}
                          type="button"
      ...
      <div className="multi-select-grid">
        {preferences.locations.map(loc => (
          <button
            ...
            className="multi-tag selected purple-selected"
          >
            <CheckCircle className="w-4 h-4" />
            {loc}
            <X className="w-3 h-3" style={{marginLeft: '4px'}} />
    ...
    <div ref={locationRef} style={{position: 'relative'}}>
      <div className="input-wrapper">
        ...
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d={locationDropdownOpen ? "M18 15l-6-6-6 6" : "M6 9l6 6 6-6"} strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
      </div>
      ...
      {locationDropdownOpen && (
        <div className="dropdown-list">
          {locations
            .filter(l => l !== "Select location..." &&
              l.toLowerCase().includes(locationSearch.toLowerCase()) &&
              !preferences.locations.includes(l))
            .map(loc => (
              <button
                key={loc}
                type="button"
                ...
                onMouseDown={(e) => {
                  e.preventDefault();
                  handleMultiToggle('locations', loc);
                  setLocationSearch("");
                }}
              >
                {loc}
